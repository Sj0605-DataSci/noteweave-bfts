...
def get_ai_client(model: str, max_retries=2) -> openai.OpenAI:
    if "ollama" in model:
        ...
        print("hitting actual openai api")
...
def query(
    ...
    **model_kwargs,
    ...
    # Strip provider prefixes (e.g. "ollama/..." or "openai/...")
    raw_model = filtered_kwargs.get("model", "")
    if "/" in raw_model:
        filtered_kwargs["model"] = raw_model.split("/", 1)[1]
