    ...
    def _prompt_impl_guideline(self):
        ...
        impl_guideline.extend(
            [
                ...
                "  - WRITE MODULAR CODE: break your solution into well-named functions (e.g. load_data(), build_model(), train(), evaluate(), plot_results()). Do NOT write a single monolithic block.",
                "  - Each function should have a single clear responsibility. Call them in sequence at the bottom of the script.",
                "  - You have full terminal access via subprocess. Use it to inspect files, list directories, grep for patterns, etc.",
                "  - INSPECTION TOOLS available via subprocess:",
                "    - List files:       subprocess.run(['ls', '-la', working_dir], capture_output=True, text=True).stdout",
                "    - Grep in files:    subprocess.run(['grep', '-r', 'pattern', working_dir], capture_output=True, text=True).stdout",
                "    - Read a file:      open(path).read()",
                "    - Find files:       subprocess.run(['find', working_dir, '-name', '*.csv'], capture_output=True, text=True).stdout",
                "    - File size/info:   os.path.getsize(path), os.stat(path)",
                "  - ALWAYS inspect your data files before using them: print shape, dtypes, first few rows.",
                "  - If a DatasetCode block is provided at the top of the script, use those variables (_dataset, _DATA_DIR, etc.) — do not re-download.",
    ...
    def plan_and_code_query(self, prompt, retries=3) -> tuple[str, str]:
        ...
        for _ in range(retries):
            ...
            if code and nl_text:
                ...
                return nl_text, code
            if code and not nl_text:
                # Model returned code-only (no preamble), e.g. thinking model with stripped <think> tags
                return "Implementation plan.", code
...
class ParallelAgent:
    ...
    def _process_node_wrapper(
        ...
        try:
            ...
            # Show generated code path and ask for confirmation before running
            runfile_path = os.path.join(workspace, cfg.exec.agent_file_name)
            with open(runfile_path, "w") as f:
                f.write(child_node.code)
            print(f"\n[bold cyan]Generated code saved to:[/bold cyan] {runfile_path}")
            print("[bold yellow]Review the code above, then press Enter to run (or type 'skip' to abort):[/bold yellow] ", end="", flush=True)
            try:
                user_input = input().strip().lower()
            except EOFError:
                user_input = ""
            if user_input == "skip":
                print("[yellow]Skipped by user.[/yellow]")
                return None
