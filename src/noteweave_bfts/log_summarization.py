...
from .llm_compat import get_response_from_llm, extract_json_between_markers
from .backend import get_ai_client
from .prompts.summarization import (
    REPORT_SUMMARIZER_SYS_MSG as report_summarizer_sys_msg,
    REPORT_SUMMARIZER_PROMPT as report_summarizer_prompt,
    STAGE_AGGREGATE_PROMPT as stage_aggregate_prompt,
    OVERALL_PLAN_SUMMARIZER_PROMPT as overall_plan_summarizer_prompt,
)
...
def overall_summarize(journals, cfg=None):
    ...
    # Pad results to 4 in case fewer stages ran (e.g. experiment failed early)
    while len(results) < 4:
        results.append(None)
